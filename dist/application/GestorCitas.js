...
class GestorCitas {
    ...
    static guardarCita(especialidad, contenido) {
        this.inicializar();
    ...
    static horariosOcupados(nombreDoctor, fecha) {
        this.inicializar();
        ...
        for (const archivo of archivos) {
            ...
            for (const linea of lineas) {
                if (linea.includes(`Doctor: ${nombreDoctor}`) && linea.includes(`Fecha: ${fecha}`)) {
                    ...
                    if (match)
                        ocupados.push(match[1]);
                }
            }
        }
        return ocupados;
    }
    static buscarCitaPorDNI(dni) {
        this.inicializar();
        const archivos = fs.readdirSync(CARPETA);
        for (const archivo of archivos) {
            const ruta = path.join(CARPETA, archivo);
    ...
    static reprogramarCita(dni, nuevaFecha, nuevaHora) {
        this.inicializar();
        const archivos = fs.readdirSync(CARPETA);
        for (const archivo of archivos) {
            const ruta = path.join(CARPETA, archivo);
    ...
    static tieneCitaDuplicada(dni, nombreDoctor, fecha) {
        this.inicializar();
        const archivos = fs.readdirSync(CARPETA);
        for (const archivo of archivos) {
            const ruta = path.join(CARPETA, archivo);
...
exports.GestorCitas = GestorCitas;
